...
from scanner.models import AiReport, CodeFinding, Dependency, GitHubScan, Vulnerability
...
class GitHubScanAdmin(ModelAdmin):
    ...
    @display(description="Source", ordering="scan_source", label={"github": "info", "local": "success"})
    ...
    @display(
        ...
        label={"pending": "info", "scanning": "warning", "completed": "success", "failed": "danger"},
    )
...
class DependencyAdmin(ModelAdmin):
    list_display = ["name", "version", "ecosystem", "scan_repo", "show_vulnerable"]
    ...
    @display(description="Vulnerable", ordering="is_vulnerable", label={True: "danger", False: "success"})
...
class VulnerabilityAdmin(ModelAdmin):
    list_display = ["cve_id", "osv_id", "show_severity", "cvss_score", "dependency_name", "fixed_version"]
    list_filter = ["severity"]
    search_fields = ["cve_id", "osv_id", "summary", "dependency__name", "dependency__scan__repo_name"]
    readonly_fields = ["dependency", "cve_id", "cvss_score", "severity", "summary", "fixed_version", "osv_id"]
    list_per_page = 50
    ...
    @display(
        ...
        ordering="severity",
        label={"critical": "danger", "high": "danger", "moderate": "warning", "medium": "warning", "low": "info"},
    )
...
class AiReportAdmin(ModelAdmin):
    ...
    readonly_fields = ["scan", "executive_summary", "priority_ranking", "remediation_json", "generated_at"]
    list_per_page = 25
...
class CodeFindingAdmin(ModelAdmin):
    list_display = ["title", "show_severity", "category", "file_path", "line_number", "scan_repo"]
    list_filter = ["severity", "category"]
    search_fields = ["title", "description", "file_path", "category", "scan__repo_name"]
    readonly_fields = [
        "scan",
        "file_path",
        "line_number",
    ...
    @display(
        description="Severity",
        ordering="severity",
        label={"critical": "danger", "high": "danger", "medium": "warning", "moderate": "warning", "low": "info", "info": "info"},
    )
    def show_severity(self, obj):
        return obj.severity
