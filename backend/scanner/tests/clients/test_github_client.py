...
from scanner.services.clients.github_client import GitHubRepositoryFetchError, _get_repo_tree, get_source_files


class TestGitHubSourceFiles:
    @patch(
        "scanner.services.clients.github_client._fetch_repo_files",
        return_value={"AppDelegate.swift": "import SwiftUI"},
    )
    @patch(
        "scanner.services.clients.github_client._get_repo_tree",
        return_value=[
    ...
    @patch("scanner.services.clients.github_client.requests.get")
